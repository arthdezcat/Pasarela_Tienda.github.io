...
const $d = document;
const $equipos = $d.getElementById("equipos");
const $template = $d.getElementById("equipo-template").content;
...
.then(json => {
    ...
    prices.forEach(el => {
        ...
        $template.querySelector(".equipo").setAttribute("data-price", el.id);
    ...
    $equipos.appendChild($fragment);
})
.catch(error => {
    let message = error.statuText || "Ocurrió un error en la petición";

    $equipos.innerHTML = `Error: ${error.status}: ${message}`;
})

$d.addEventListener("click", e => {
    if (e.target.matches(".equipos *")) {
        ...
        .then(res => {
            if (res.error){
                $equipos.insertAdjacentElement("afterend", res.error.message)
            }
        })
    }
})
